...
- Use a fresh `worker` as sole writer in its isolated worktree only when delegation is useful. Workers own bounded regression tests and implementation; parallel workers require non-overlapping ownership. Use one fresh `reviewer` after meaningful changes.
